...
class kehu2TableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        getRecodFromCloud()
        let item = UIBarButtonItem(title: "添加客户", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(kehu2TableViewController.add))
    ...
    func add() {
        ...
        navigationController?.pushViewController(AddKehuTableViewController(), animated: true)
...
//        let vc: kehuInfoViewController = kehuInfoViewController()
        let vc: kehuInfoTableViewController = kehuInfoTableViewController()
        let data = kehudata[indexPath.row]
